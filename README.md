...
- `select_toolchain.sh`: Interactive script to switch between different toolchain variants (XBurst1 vs XBurst2, different C libraries). Selection supports `0` for all toolchains or space-separated choices like `1 3 4`; the last selected entry becomes active. It also checks whether selected remote bundles are newer and reinstalls automatically when needed.
